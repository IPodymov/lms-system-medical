...
class EnrollmentView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            item = enroll(
                course_run=get_object_or_404(CourseRun, pk=request.data.get("course_run")),
                user=request.user,
            )
...
class StartAttemptView(APIView):
    def post(self, request, quiz_id):
        ...
        try:
            return Response(
                AttemptSerializer(start_attempt(quiz=quiz, enrollment=enrollment)).data, 201
            )
...
class AnswerView(APIView):
    def put(self, request, attempt_id):
        ...
        except (QuizError, KeyError) as exc:
            return Response({"detail": str(exc)}, 400)
